...
function validManifest(): ApplicationContentManifest {
  return {
    ...
    releases: APPLICATION_LOCALE_CANDIDATES.map(({ tag }) => ({
...
test('accepts an exact 41-locale application content manifest', () => {
  const manifest = validateApplicationContentManifest(validManifest());
  assert.equal(manifest.releases.length, 41);
  assert.equal(applicationContentReleaseMap(manifest).size, 41);
});
