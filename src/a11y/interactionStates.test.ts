...
test('Reduce Motion replaces the completion shimmer with the static done state', () => {
  ...
  // The sweep must not start at all, or the shared value animates unseen.
  assert.match(card, /if \(reduceMotion\) return;/);
  assert.match(card, /entering=\{reduceMotion \? undefined : ZoomIn/);
  assert.match(card, /name="checkmark-circle"/);

  // What the name of this test promises, which it did not previously check:
  // gating the sweep on its own leaves a completed card with nothing where the
  // reward was, so a still gold wash takes its place (roadmap item 37).
  assert.match(card, /reduceMotion \? \(\s*done \? \(\s*<LinearGradient/);
});
